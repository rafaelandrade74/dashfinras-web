...
talks to Supabase Auth and to `api-dashfinras`'s `account/login`/`account/logout`**, and it sets a
same-origin httpOnly cookie for the browser. No `SameSite=None` needed anywhere.
...
- `src/server/auth.ts` (`createAuthRouter`) implements `POST /api/auth/login|signup|reset-password|logout`
  and `GET /api/auth/session`. `login`/`signup` call Supabase Auth server-side, then call
  `api-dashfinras`'s `POST account/login` server-to-server (`Authorization: Bearer <accessToken>`,
  same contract the browser used to call directly) — its `Set-Cookie` response is ignored. On
  success, the access/refresh token pair is sealed (via `iron-session`) into this app's own
  same-origin cookie, `df_session` (`httpOnly`, `Secure`, `SameSite=Lax`).
...
- `src/server/api-proxy.ts` (`createApiProxy`) is a generic `http-proxy-middleware` proxy mounted at
  ...
  the browser only ever talks to this app's own origin, never to `api-dashfinras` directly.
- Dependencies (`supabaseAdmin` client, `fetch`) are injected into `createAuthRouter`/
  `createApiProxy`/`createApiApp` rather than imported as singletons, specifically so
  `src/server/auth.spec.ts` can pass test doubles — the Angular test runner blocks `vi.mock()` on
  relative imports.
...
The API (`api-dashfinras`, .NET) lives as a sibling directory and has its own CLAUDE.md. Its
