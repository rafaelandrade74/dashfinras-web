...
import { TipoMovimentacao } from './movimentacao-financeira-temp.model';
...
describe('RegistrarMovimentacaoModal', () => {
  ...
  describe('submissão', () => {
    it('registra com sucesso, converte competência/valor e fecha o modal', () => {
      ...
      const req = httpMock.expectOne('/api/movimentacao-financeira');
    ...
    it('envia tags e observação quando preenchidos', () => {
      abrirModal();
      preencherFormularioValido();
      component.form.controls['tags'].setValue(['recorrente', 'cartão']);
      component.form.controls['observacao'].setValue('  parcela 3 de 6  ');

      component.registrar();

      const req = httpMock.expectOne('/api/movimentacao-financeira');
      expect(req.request.body.tags).toEqual(['recorrente', 'cartão']);
