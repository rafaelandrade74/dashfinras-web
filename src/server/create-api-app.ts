 ...
 *
 * A dependência (client Supabase) é injetável, com default real — permite testar a lógica de
 * auth/proxy com dublês, sem mocking de módulo (bloqueado pelo test runner do Angular para
 * imports relativos).
 */
export function createApiApp(options: CreateApiAppOptions = {}): Express {
  ...
  app.use('/api/auth', express.json(), createAuthRouter({ supabaseAdmin }));
